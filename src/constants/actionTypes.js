// timer
export const TYPE_SET = 'TYPE_SET';
export const SCRAMBLE_GENERATED = 'SCRAMBLE_GENERATED';
export const STATE_SET = 'STATE_SET';
export const INSPECTION_STARTED = 'INSPECTION_STARTED';
export const CURRENT_PENALTY_SET = 'CURRENT_PENALTY_SET';
export const HOLDING_STARTED = 'HOLDING_STARTED';
export const HOLDING_STOPPED = 'HOLDING_STOPPED';
export const TIMER_STARTED = 'TIMER_STARTED';
export const TIMER_STOPPED = 'TIMER_STOPPED';
export const SPACEBAR_DOWN_SET = 'SPACEBAR_DOWN_SET';
export const TOUCH_DOWN_SET = 'TOUCH_DOWN_SET';
export const SOLVE_DELETED = 'SOLVE_DELETED';
export const SESSION_CREATED = 'SESSION_CREATED';
export const SESSION_SWITCHED = 'SESSION_SWITCHED';
export const CURRENT_SESSION_DELETED = 'CURRENT_SESSION_DELETED';
export const PENALTY_SET = 'PENALTY_SET';
export const COLORS_SET = 'COLORS_SET';
export const COLOR_SCHEME_SET = 'COLOR_SCHEME_SET';
export const GLOW_SET = 'GLOW_SET';
export const FROM_LOCAL_STORAGE_INITTED = 'FROM_LOCAL_STORAGE_INITTED';

// settings
export const INSPECTION_SET = 'INSPECTION_SET';
export const HOLD_TIME_SET = 'HOLD_TIME_SET';
export const DISPLAY_MILLIS_SET = 'DISPLAY_MILLIS_SET';
export const HIDE_SOLVE_TIME_SET = 'HIDE_SOLVE_TIME_SET';
export const SHOW_TIMES_SET = 'SHOW_TIMES_SET';

// modal
export const MODAL_CREATED = 'MODAL_CREATED';
export const MODAL_REMOVED = 'MODAL_REMOVED';
export const MODAL_STATE_SET = 'MODAL_STATE_SET';